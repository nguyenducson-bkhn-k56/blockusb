        ...
        public Boolean editFileDevice(List<Device> lstDevice){
            ...
            try
            {
                ...
                if (!Directory.Exists(pathAppfolder + nameFolder)) {
                    DirectoryInfo isCreated = Directory.CreateDirectory(pathAppfolder + nameFolder);
                }
                Boolean flag = File.Exists(filePath);
                if(flag)
                {
                    // remove all file 
                    File.WriteAllText(filePath, String.Empty);
                }
                using ( writer = File.Exists(filePath) ? File.AppendText(filePath): File.CreateText(filePath)) {
